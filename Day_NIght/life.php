...
// Last update Wed Dec  4 11:18:36 2013 Alexandre LEVY
//
...
function evolve(&$grid)
{
  for ($j = 0; $j < Y; ++$j) {
    for ($i = 0; $i < X; ++$i) {
      if ($grid[$j][$i] == 0)
	{
	  if (check_around($grid, $i, $j) == 3 || check_around($grid, $i , $j) >= 6 && check_around($grid, $i, $j) <= 8)
	    $grid2[$j][$i] = 1;
	  else
	    $grid2[$j][$i] = 0;
	}
	...
	{
	  if (check_around($grid, $i, $j) < 3 || check_around($grid, $i, $j) > 8 || check_around($grid, $i, $j) == 5)
	    $grid2[$j][$i] = 0;
	  else
	    $grid2[$j][$i] = $grid[$j][$i];
	}
